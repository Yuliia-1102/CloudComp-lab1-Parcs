...
class Solver:
    ...
    def solve(self):
        ...
        result_matrix = self.reduce(mapped, n, bs)
    ...
    def compute(bs, a_slice, matrix_b, start_block, end_block):
        ...
        num_block_rows = end_block - start_block
        rows_for_worker = num_block_rows * bs

        local_c = [[0] * n for _ in range(rows_for_worker)]
                ...
                for bk in range(nb):
                    for i in range(bs):
                        for j in range(bs):
                            ...
                            global_row_c = bi * bs + i
                            global_col_c = bj * bs + j

                            local_row_c = global_row_c - start_block * bs
                            local_c[local_row_c][global_col_c] += sum_val

        return (start_block, local_c)

    @staticmethod
    @expose
    def reduce(mapped, n, bs):
        print("reduce")
        result_matrix = [[0] * n for _ in range(n)]

        for worker_result in mapped:
            start_block, local_c = worker_result.value
            start_row = start_block * bs

            for i, row in enumerate(local_c):
